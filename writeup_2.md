...
* **מצב התחלתי – בדיקת דגל**
בתחילת השלב מתבצעת בדיקה של ערך ברגיסטר eax. תחילה מועבר הערך 1 אל eax, כלומר – מוגדר דגל הצלחה כברירת מחדל.
לאחר מכן מופעלת הפקודה test eax, eax, שבודקת האם תוכן eax הוא אפס (0) באמצעות AND ביטי של הרגיסטר עם עצמו. הפקודה משפיעה על דגל האפס (ZF) אך אינה משנה את ערך eax.

מיד לאחר מכן, פקודת הקפיצה jz (Jump if Zero) תקפוץ לכתובת loc_401427 אם תוצאת הבדיקה היא אפס, כלומר – אם eax שווה ל־0.
בכתובת זו מתבצע סיום הפונקציה: שחזור של ערכי esp ו־ebp ופקודת retn שמחזירה את הבקרה למקום הקריאה.

מכיוון שהערך ההתחלתי של eax הוא 1, הקפיצה לא מתבצעת והקוד ממשיך לפעול ברצף.
