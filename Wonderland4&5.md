...
```
* אם המיקום אליו זזנו מכיל נקודה (0x2E), נמשיך להריץ את הלולאה ולבצע את הצעד הבא בקלט.
...
* אם לא הגענו ל־'X', הלולאה מסתיימת עם כישלון (ערך 0).
  ...
  * **לסיכום** בכל צעד בלולאה, הקוד קורא את התו במיקום החדש שאליו הגענו לאחר ביצוע תנועה.
...
1. **זיהוי המיקומים** - מניתוח הדאטה הגולמית:
  ...
  ![4](https://github.com/shirelsan/Assembly-Reversing/blob/main/image_wonder4+5/wonder5_1.jpg?raw=true) 
2. מיקומים חשובים:

* התו **O** (התחלה): **0x404033**
* התו **X** (יעד): **0x40406D**

3. סקרייפ פייתון שמקבל את הדאטה והלוגיקה של ההזזות ומוצא את המחרוזת הנכון שצריך להכניס לקובץ: 
```python
hex_data = [
    "23 23 23 4F 23 23 23 23 2E 2E 2E 23 23 23 23 23",
    "23 23 23 2E 23 23 2E 2E 2E 23 2E 23 23 23 23 23",
    "23 23 23 2E 23 23 2E 23 23 23 2E 23 23 23 23 23",
    "23 23 23 2E 2E 2E 2E 23 23 23 2E 2E 2E 58 23 23"]
maze, start, end = [], None, None
for r, row_data in enumerate(hex_data):
    clean = row_data.replace(" ", "")
    row = []
    for i in range(0, len(clean), 2):
        c = int(clean[i:i+2], 16)
        if c == 0x4F: start = (r, len(row)); row.append('O')
        elif c == 0x58: end = (r, len(row)); row.append('X')
        elif c == 0x2E: row.append('.')
        else: row.append('#')
    maze.append(row)
    ...
    while q:
        ...
        for i,(dr,dc) in enumerate(directions):
            ...
            if 0 <= nr < rows and 0 <= nc < cols and (nr,nc) not in visited:
                if m[nr][nc] in ['.', 'X']:
                    visited.add((nr,nc))
                    q.append((nr,nc,p+names[i]))
    return None

if start and end:
    sol = find_path(maze, start, end)
    if sol:
        print(f"Path sequence: {sol}")
```
מה הסקרייפ עושה? 

מקבל רשימת מחרוזות hex_data שמייצגות קוד הקסדסימלי של מבוך בתווים (עם תווים כמו #, ., O, X).

ממיר כל שורה ב־hex_data למערך דו-ממדי maze של תווים:

  * התו O — נקודת התחלה (מוצאת את המיקום שלה)

  * התו X — נקודת סיום (מוצאת את המיקום שלה)

  * התו . — דרך פתוחה במבוך

  * התו # — מקום חסום

מגדיר פונקציה find_path שמבצעת חיפוש מסלול במבוך בין נקודת ההתחלה לנקודת הסיום:

  * משתמשת באלגוריתם BFS (חיפוש לרוחב) כדי למצוא דרך חוקית לעבור רק דרך תווים . או היעד X.

  * בכל צעד בודקת את 4 הכיוונים האפשריים (ימינה, למטה, שמאלה, למעלה).

  * מתעדת את המסלול שנמצא באמצעות רצף של מספרים המייצגים את הכיוונים:

  1 = ימינה, 2 = למטה, 3 = שמאלה, 4 = למעלה

בסוף, אם נמצא מסלול חוקי מ־O ל־X, מדפיסה את רצף הצעדים (המחרוזת).
...
* לאחר הבדיקה של הפונקציה היא מחזירה את eax אם הוא שווה 1 הפונקציה תעבור למסר ההצלחה ואם הוא 0 יודפס מסר כישלון
  ...
  ![4](https://github.com/shirelsan/Assembly-Reversing/blob/main/image_wonder4+5/wonder5_4.jpg?raw=true) 
