...
####ניתוח פונקציית hack_security והשגת מסר ההצלחה####
***פתיחת קובץ הקלט וקריאת נתונים**
* בשלב זה, פונקציית hack_security פותחת את הקובץ הבא: C:\ReversingCTF\DroneAttack.txt
* מתוך קובץ זה מתבצעת קריאה של 8 ערכים בפורמט הקסדצימלי (Hex), באמצעות הפורמט %x. הערכים שנקראים נשמרים בזיכרון באיזור המשתנה var_24.
  
* **עיבוד הערכים באמצעות sub_10001220**
* עבור כל אחד מה-8 הערכים שנקראו, מתבצעת קריאה לפונקציה sub_10001220.
* הערך המוחזר מכל קריאה שכזו נשמר לתוך מערך בזיכרון שנמצא בכתובת: dword_10004378
כלומר, מערך זה בסופו של דבר מכיל את 8 הערכים שעברו המרה על-ידי sub_10001220.

* **בדיקת קוד ע"י sub_10001250**
בהמשך הריצה, אני מתבקשת להזין קוד באורך 8 ספרות. הקלט של המשתמש נבדק באמצעות הפונקציה sub_10001250.

פונקציה זו אחראית לוודא שהקלט עומד בכל התנאים שנדרשים לטובת הדספת מסר ההצלחה. 

### ניתוח פונקציה: sub_10001220
**מטרת:**
הפונקציה מקבלת מספר שלם (32־ביט) כפרמטר קלט, מבצעת עליו פעולה ומחזירה ערך חדש.
הפונקציה משמשת לעיבוד הערכים שנקראו מהקובץ לפני השימוש בהם בפונקציה הבאה שתאמת את הקוד (sub_10001250).

* הפונקציה משתמשת במשתנה var_4 - בו היא תאחסן את התוצאה הסופית.
* ישנה לולאת עיבוד הביטים
 * השורה and eax, 1 בודקת האם הביט הנמוך ביותר של הקלט דלוק (1) או כבוי (0).
 * אם דלוק – מוסיף 1 ל־var_4, שהוא הספירה הכוללת.
 * לאחר מכן מבצע הזזה ימינה (shr) של הקלט ב־1, כלומר מקדם לביט הבא.
 * התהליך חוזר עד שכל הביטים של הקלט עברו עיבוד.
 * 
בסיום הפונקציה, מוחזר הערך שנשמר ב־var_4, שהוא מספר הביטים הדלוקים בייצוג הבינארי של הקלט.

הקובץ מכיל את הערכים הבאים: 
EADC CB51 BEC3 D8C3 DA53 7C92 788A 474C
באמצעות אנליזה דינמית, פלט החישוב של מספר הביטים הדלוקים עבור כל אחד מהערכים שהוזנו בקובץ:
