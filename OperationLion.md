...
לכן נסיתי לחקור את הפונקציה וכשמתי breakpoint על השורה של ההשוואה בשביל לראות את התנאי.

כיוון שה-breakpoint לא עבד והתוכנית ישר נסגרה. הבנתי שככה"נ יש פה מנגנון אנטי-דיבג.

5. **ניתוח DllMain:**
* הפונקציה בודקת אם fdwReason == 1 כלומר האם ה־DLL נטען.
*  אם כן, היא קוראת לפונקציה sub_10001200.
*  אם הפונקציה לא מחזירה 1, מודפס "Pilot, mission failed..." ואז ```call ExitProcess ```

  כלומר, התהליך נסגר לפני שאני מגיעה ל־hack_security
  
6. **ניתוח הפונקציה sub_10001200 על מנת לגרום לה להחזיר תמיד 0:**
